...
// Route: /api/gap-in-meeting
// Feature: In Meeting
// Description: In-meeting realtime overlay (live transcription + suggestions)
  ...
  try {
    const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: 'You are an expert assistant for: In Meeting. In-meeting realtime overlay (live transcription + suggestions)' },
...
router.post('/', async (req: Request, res: Response) => {
  try {
    ...
    return res.json({ feature: 'in-meeting', title: 'In Meeting', result, generatedAt: new Date().toISOString() });
  } catch (e: any) {
...
router.get('/history', async (req: Request, res: Response) => {
  try {
    const db = req.app && (req.app.get ? req.app.get('db') : null);
    if (!db) return res.json({ slug: 'in-meeting', items: [] });
    if (typeof db.query === 'function') {
      const r = await db.query('SELECT id, input, output, created_at FROM gap_features WHERE slug = $1 ORDER BY id DESC LIMIT 20', ['in-meeting']);
      return res.json({ slug: 'in-meeting', items: r.rows || [] });
    }
    return res.json({ slug: 'in-meeting', items: [] });
  } catch (e: any) {
    return res.json({ slug: 'in-meeting', items: [], error: (e && e.message) || 'Server error' });
  }
});
