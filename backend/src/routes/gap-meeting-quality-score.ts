...
// Route: /api/gap-meeting-quality-score
// Feature: Meeting Quality Score
  ...
  try {
    const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: 'You are an expert assistant for: Meeting Quality Score. (effectiveness scoring)' },
...
router.post('/', async (req: Request, res: Response) => {
  try {
    ...
    const prompt = `Feature: Meeting Quality Score\nDescription: (effectiveness scoring)\nInput: ${input}\nContext: ${JSON.stringify(context || {})}`;
    const result = await callOpenRouter(prompt);
    // Best-effort persistence
    try {
      if (db && typeof db.query === 'function') {
        await db.query('INSERT INTO gap_features (slug, input, output, meta) VALUES ($1, $2, $3, $4)',
          ['meeting-quality-score', input, JSON.stringify(result), JSON.stringify(context || {})]);
    ...
    return res.json({ feature: 'meeting-quality-score', title: 'Meeting Quality Score', result, generatedAt: new Date().toISOString() });
  } catch (e: any) {
...
router.get('/history', async (req: Request, res: Response) => {
  try {
    const db = req.app && (req.app.get ? req.app.get('db') : null);
    if (!db) return res.json({ slug: 'meeting-quality-score', items: [] });
    if (typeof db.query === 'function') {
      const r = await db.query('SELECT id, input, output, created_at FROM gap_features WHERE slug = $1 ORDER BY id DESC LIMIT 20', ['meeting-quality-score']);
      return res.json({ slug: 'meeting-quality-score', items: r.rows || [] });
    }
    return res.json({ slug: 'meeting-quality-score', items: [] });
  } catch (e: any) {
    return res.json({ slug: 'meeting-quality-score', items: [], error: (e && e.message) || 'Server error' });
  }
});
