...
// Route: /api/gap-decision-consensus-check
// Feature: Decision Consensus Check
// Description: (disagreement detection)
  ...
  try {
    const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: 'You are an expert assistant for: Decision Consensus Check. (disagreement detection)' },
...
router.post('/', async (req: Request, res: Response) => {
  try {
    ...
    const prompt = `Feature: Decision Consensus Check\nDescription: (disagreement detection)\nInput: ${input}\nContext: ${JSON.stringify(context || {})}`;
    const result = await callOpenRouter(prompt);
    // Best-effort persistence
    try {
      if (db && typeof db.query === 'function') {
        await db.query('INSERT INTO gap_features (slug, input, output, meta) VALUES ($1, $2, $3, $4)',
          ['decision-consensus-check', input, JSON.stringify(result), JSON.stringify(context || {})]);
    ...
    return res.json({ feature: 'decision-consensus-check', title: 'Decision Consensus Check', result, generatedAt: new Date().toISOString() });
  } catch (e: any) {
...
router.get('/history', async (req: Request, res: Response) => {
  try {
    const db = req.app && (req.app.get ? req.app.get('db') : null);
    if (!db) return res.json({ slug: 'decision-consensus-check', items: [] });
    if (typeof db.query === 'function') {
      const r = await db.query('SELECT id, input, output, created_at FROM gap_features WHERE slug = $1 ORDER BY id DESC LIMIT 20', ['decision-consensus-check']);
      return res.json({ slug: 'decision-consensus-check', items: r.rows || [] });
    }
    return res.json({ slug: 'decision-consensus-check', items: [] });
  } catch (e: any) {
    return res.json({ slug: 'decision-consensus-check', items: [], error: (e && e.message) || 'Server error' });
  }
});
