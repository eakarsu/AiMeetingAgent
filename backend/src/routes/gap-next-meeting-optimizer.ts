...
// Route: /api/gap-next-meeting-optimizer
// Feature: Next Meeting Optimizer
// Description: (date/time suggestion)
  ...
  try {
    const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: 'You are an expert assistant for: Next Meeting Optimizer. (date/time suggestion)' },
...
router.post('/', async (req: Request, res: Response) => {
  try {
    ...
    const prompt = `Feature: Next Meeting Optimizer\nDescription: (date/time suggestion)\nInput: ${input}\nContext: ${JSON.stringify(context || {})}`;
    const result = await callOpenRouter(prompt);
    // Best-effort persistence
    try {
      if (db && typeof db.query === 'function') {
        await db.query('INSERT INTO gap_features (slug, input, output, meta) VALUES ($1, $2, $3, $4)',
          ['next-meeting-optimizer', input, JSON.stringify(result), JSON.stringify(context || {})]);
    ...
    return res.json({ feature: 'next-meeting-optimizer', title: 'Next Meeting Optimizer', result, generatedAt: new Date().toISOString() });
  } catch (e: any) {
...
router.get('/history', async (req: Request, res: Response) => {
  try {
    const db = req.app && (req.app.get ? req.app.get('db') : null);
    if (!db) return res.json({ slug: 'next-meeting-optimizer', items: [] });
    if (typeof db.query === 'function') {
      const r = await db.query('SELECT id, input, output, created_at FROM gap_features WHERE slug = $1 ORDER BY id DESC LIMIT 20', ['next-meeting-optimizer']);
      return res.json({ slug: 'next-meeting-optimizer', items: r.rows || [] });
    }
    return res.json({ slug: 'next-meeting-optimizer', items: [] });
  } catch (e: any) {
    return res.json({ slug: 'next-meeting-optimizer', items: [], error: (e && e.message) || 'Server error' });
  }
});
