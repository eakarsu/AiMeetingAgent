...
// Route: /api/gap-full-text
// Feature: Full Text
// Description: Full-text search across meeting library
  ...
  try {
    const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: 'You are an expert assistant for: Full Text. Full-text search across meeting library' },
...
router.post('/', async (req: Request, res: Response) => {
  try {
    ...
    return res.json({ feature: 'full-text', title: 'Full Text', result, generatedAt: new Date().toISOString() });
  } catch (e: any) {
...
router.get('/history', async (req: Request, res: Response) => {
  try {
    const db = req.app && (req.app.get ? req.app.get('db') : null);
    if (!db) return res.json({ slug: 'full-text', items: [] });
    if (typeof db.query === 'function') {
      const r = await db.query('SELECT id, input, output, created_at FROM gap_features WHERE slug = $1 ORDER BY id DESC LIMIT 20', ['full-text']);
      return res.json({ slug: 'full-text', items: r.rows || [] });
    }
    return res.json({ slug: 'full-text', items: [] });
  } catch (e: any) {
    return res.json({ slug: 'full-text', items: [], error: (e && e.message) || 'Server error' });
  }
});
