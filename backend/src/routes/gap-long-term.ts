...
// Route: /api/gap-long-term
// Feature: Long Term
// Description: Long-term analytics dashboard (trends)
  ...
  try {
    const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: 'You are an expert assistant for: Long Term. Long-term analytics dashboard (trends)' },
...
router.post('/', async (req: Request, res: Response) => {
  try {
    ...
    return res.json({ feature: 'long-term', title: 'Long Term', result, generatedAt: new Date().toISOString() });
  } catch (e: any) {
...
router.get('/history', async (req: Request, res: Response) => {
  try {
    const db = req.app && (req.app.get ? req.app.get('db') : null);
    if (!db) return res.json({ slug: 'long-term', items: [] });
    if (typeof db.query === 'function') {
      const r = await db.query('SELECT id, input, output, created_at FROM gap_features WHERE slug = $1 ORDER BY id DESC LIMIT 20', ['long-term']);
      return res.json({ slug: 'long-term', items: r.rows || [] });
    }
    return res.json({ slug: 'long-term', items: [] });
  } catch (e: any) {
    return res.json({ slug: 'long-term', items: [], error: (e && e.message) || 'Server error' });
  }
});
